...
public class ExceptionInfoHandler {
    ...
    public ErrorInfo bindingError(HttpServletRequest req, BindException e) {
        String[] details = e.getBindingResult().getFieldErrors().stream()
                .map(ex -> String.format("%s %s", ex.getField(), ex.getDefaultMessage())).toArray(String[]::new);
    ...
    private static ErrorInfo logAndGetErrorInfo(HttpServletRequest req, Exception e, boolean logException, ErrorType errorType, String... details) {
        ...
        return new ErrorInfo(req.getRequestURL(), errorType, details.length == 0 ? Collections.singletonList(rootCause.toString()) : List.of(details));
    }
}